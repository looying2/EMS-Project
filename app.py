...
if user_role == "Doctor":
    ...
    with tab_records:
        ...
        st.markdown("_Real‑time EMG amplitude during the session (smoothed with 20‑point moving average). Colour indicates muscle state: 🔴 overexertion, 🟡 fatigue, 🟢 moderate, 🔵 relaxed._")
        ...
        # Summary stats row
        _avg_emg = np.mean(_emg_vals)
        _max_emg = max(_emg_vals)
        _n_emg = len(_emg_vals)
        _dom_state = max(set(_emg_states), key=_emg_states.count).title()
        ec1, ec2, ec3, ec4 = st.columns(4)
        ec1.metric("📊 Readings", f"{_n_emg}")
        ec2.metric("📈 Avg EMG", f"{_avg_emg:.0f} µV")
        ec3.metric("⬆ Peak", f"{_max_emg} µV")
        ec4.metric("🏃 Dominant State", _dom_state)
