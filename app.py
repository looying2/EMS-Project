...
def fb_write(path, data):
    """Write (PUT) data to Firebase via REST."""
...
def fb_read(path):
    """Read data from Firebase via REST."""
    url = f"{FIREBASE_URL}/{path}.json?auth={FIREBASE_SECRET}"
    try:
...
def fb_push(path, data):
    """Push (POST) data to Firebase — auto-generated key."""
    url = f"{FIREBASE_URL}/{path}.json?auth={FIREBASE_SECRET}"
    try:
...
def generate_session_summary():
    ...
    # Save session summary + metadata to Firebase
    ...
    fb_write(f"patients/{patient_id}/sessions/{session_id}", {
        ...
        "avg_emg":           float(tele_snap["emg"].mean()) if not tele_snap.empty else 0,
        "max_emg":           float(tele_snap["emg"].max())  if not tele_snap.empty else 0,
        "pain_score":        st.session_state.live_pain,
        "fatigue_score":     st.session_state.live_fatigue,
        "ml_classification": st.session_state.ml_prediction,
        "ml_confidence":     float(st.session_state.ml_probability * 100),
        "rag_summary":       answer if isinstance(answer, str) else json.dumps(answer),
...
with st.sidebar:
    ...
    # Save patient profile to Firebase (idempotent PUT on every rerun)
...
if user_role == "Doctor":
                ...
                with st.spinner("Extracting and analyzing report..."):
                    try:
                        ...
                        if st.button("Save to Patient Record", use_container_width=True):
                            ...
                            report_date = metrics.get("Test Date", datetime.now().strftime("%Y-%m-%d"))
                            safe_date = str(report_date).replace("/", "-").replace(" ", "_").replace(":", "")
                            bia_data = {
                                "date":                    report_date,
                            ...
                            if fb_write(f"patients/{patient_id}/bia_reports/{safe_date}", bia_data):
    ...
    with tab_records:
        ...
        # =====================================================
        # 2. SESSION PAIN & FATIGUE TRENDS (reordered earlier)
        # =====================================================
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
        ...
        with col_p1:
            ...
            _fig_pain.add_trace(go.Scatter(
                ...
                marker=dict(size=10, color="#EF5350", line=dict(color="white", width=2)),
            ...
            <div style="display:flex;gap:10px;margin-top:-6px;">
                <div style="flex:1;background:#FEF2F2;border-radius:8px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.68rem;font-weight:600;color:#94A3B8;">START</div>
                ...
                <div style="flex:1;background:#F0FDF4;border-radius:8px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.68rem;font-weight:600;color:#94A3B8;">END</div>
                ...
                <div style="flex:1;background:#F8FAFC;border-radius:8px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.68rem;font-weight:600;color:#94A3B8;">CHANGE</div>
        ...
        # =====================================================
        # 3. SEGMENTAL LEAN MASS ANALYSIS
        # =====================================================
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
        ...
        # =====================================================
        # 4. LONG-TERM MUSCULOSKELETAL HEALTH
        # =====================================================
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
